...
describe('Cinema', function () {
  ...
  it('should be able to check whether there are some films from a particular year', function(){
    const actual = cinema.checkIfAnyFilmsOfYear(2017);
    const expected = true;
    assert.strictEqual(actual, expected);
  });

  it('should be able to check whether there are no films from a particular year', function(){
    const actual = cinema.checkIfAnyFilmsOfYear(4020);
    const expected = false;
    assert.strictEqual(actual, expected);
  });

  it('should be able to check whether all films are over a particular length - returns true', function(){
    const actual = cinema.checkIfAllFilmsAreOverThisLength(95);
    const expected = true;
    assert.strictEqual(actual, expected);
  });

  it('should be able to check whether all films are over a particular length - returns false', function(){
    const actual = cinema.checkIfAllFilmsAreOverThisLength(96);
    const expected = false;
    assert.strictEqual(actual, expected);
  });
